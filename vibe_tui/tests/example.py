...
class SimpleApp:
    def __init__(self):
        ...
        pool_size = 1000
        suffix_pool = [''.join(random.choices(string.ascii_lowercase, k=10)) for _ in range(pool_size)]
        
        self.all_items = [
            f"r/{'s' if i % 3 == 0 else 'u'}/{i:02d}/{suffix_pool[i % pool_size]}.cfg"
            for i in range(30 * 1_000_000) # 30 mil Items
        ]
