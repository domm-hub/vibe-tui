...
class AdvancedFileBrowser:
    def __init__(self, start_path="."):
        ...
        # Editor Modal
        self.editor_field = PythonCode(weight=1, text="", title=" FILE EDITOR (Ctrl+S to save) ")
        ...
        self.app = App(self.root, modals=[self.help_modal, self.confirm_modal, self.input_modal, self.editor_modal, self.toast])
            ...
            try:
                ...
                if self.is_binary(full_path) and ext not in ('png', 'jpg', 'jpeg', 'svg', 'webp', 'bmp'):
                    ...
                    try:
                        ...
                        console.print(Syntax(code_content, ext if ext != 'py' else 'python', theme="monokai", line_numbers=False))
        ...
        def custom_input_handler(key):
            ...
            if key in (Key.ENTER[0], "\n"):
                if self.app.fm.current == self.file_list:
                    if not self.current_filtered_items: return True
                    _, raw_name, is_dir = self.current_filtered_items[self.file_list.selection]
                    if is_dir: self.load_directory(self.current_path / raw_name); self.search_bar.set("")
                    return True
    ...
    def go_up(self):
        if self.current_path.parent != self.current_path: self.load_directory(self.current_path.parent); self.search_bar.set("")
